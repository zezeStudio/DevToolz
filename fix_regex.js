...
code = code.replace(/const arrayRegex = new RegExp\(\`:\\s\*\$\{name\}\\s\*\\\[\\\]\`,\s*"g"\);/, "const arrayRegex = new RegExp(`:\\\\s*${name}\\\\s*\\\\[\\\\]`, \"g\");");
code = code.replace(/const typeRegex = new RegExp\(\`:\\s\*\$\{name\}\(\?\!\[a-zA-Z0-9_\$\]\)\`,\s*"g"\);/, "const typeRegex = new RegExp(`:\\\\s*${name}(?![a-zA-Z0-9_$])`, \"g\");");
