...
// WE WILL RECREATE THE EXACT LOGIC AS IT IS IN THE ACTUAL COMPONENT FILE
...
for (const name of interfaceOrder) {
  ...
  for (const otherName of interfaceOrder) {
    ...
    // With TWO backslashes inside source code!
...
function resolveBody(body, depth = 0) {
      ...
      // Primitives Arrays
      // In source we need TWO backslashes to mean `\s`. So in string literal we put `\\\\s`.
      resolved = resolved
