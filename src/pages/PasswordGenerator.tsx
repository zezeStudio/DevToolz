    ...
    <>
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        <div className="mb-8 text-center">
          ...
          <h1 className="text-4xl font-extrabold text-gray-900 tracking-tight">{t('pass.title')}</h1>
          <p className="text-gray-500 mt-3 text-lg max-w-2xl mx-auto">{t('pass.desc')}</p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                  ...
                  {passwords.length <= 1 ? (
                    <>
                      <div className="p-6 min-h-[120px] flex items-center justify-center">
                        <p className="text-xl sm:text-2xl md:text-3xl font-mono text-center text-gray-800 tracking-wider break-all select-all">
                          {showPasswords ? password : '•'.repeat(password.length)}
                        </p>
                      </div>
                      <div className="bg-gray-50 border-t border-gray-100 p-3 flex items-center justify-center gap-2">
                        <button
                          onClick={() => setShowPasswords(!showPasswords)}
                          className="flex-1 sm:flex-none flex items-center justify-center px-4 py-2.5 text-sm font-bold text-gray-600 hover:text-green-600 hover:bg-green-100 rounded-xl transition-all"
                        >
                    ...
                    <div className="flex flex-col">
                      <div className="max-h-[320px] overflow-y-auto custom-scrollbar">
                        {passwords.map((p, i) => (
                          <div key={i} className="flex flex-col sm:flex-row sm:items-center justify-between p-4 border-b border-gray-100 last:border-0 hover:bg-green-50 transition-all gap-3">
                            <code className="text-lg font-mono text-gray-700 break-all">
                              {showPasswords ? p : '•'.repeat(p.length)}
                            </code>
                            <div className="flex items-center space-x-2 self-end sm:self-auto sm:ml-4 shrink-0">
                              <button onClick={() => setShowQr(p)} className="p-2 text-gray-400 hover:text-green-600 rounded-lg transition-all bg-white border border-gray-200 sm:border-transparent sm:bg-transparent shadow-sm sm:shadow-none">
                                <QrCode className="w-5 h-5" />
                              </button>
                              <button onClick={() => copyToClipboard(p, i)} className="p-2 text-gray-400 hover:text-green-600 rounded-lg transition-all bg-white border border-gray-200 sm:border-transparent sm:bg-transparent shadow-sm sm:shadow-none">
                      ...
                      <div className="bg-gray-50 border-t border-gray-100 p-3 flex flex-col sm:flex-row items-center justify-center gap-2">
                        <button
                          onClick={() => setShowPasswords(!showPasswords)}
                          className="w-full sm:w-auto flex items-center justify-center px-6 py-2 text-sm font-bold text-gray-600 hover:text-green-600 hover:bg-green-100 rounded-xl transition-all"
                        >
                ...
                <div className="mt-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  ...
                  <div className="flex items-center justify-between sm:justify-end space-x-4">
                    <div className="text-left sm:text-right">
                      ...
                      <span className="text-sm font-mono font-bold text-gray-700">{entropy} {t('pass.bits')}</span>
                    </div>
                    <div className="flex space-x-1 h-3 w-24 sm:w-32 bg-gray-100 rounded-full overflow-hidden border border-gray-200">
              ...
              <div className="p-4 md:p-8 space-y-8">
                <div className="flex p-1 bg-gray-100 rounded-2xl">
                  <button
                    ...
                    className={`flex-1 flex items-center justify-center py-2 md:py-3 px-2 md:px-4 rounded-xl font-bold transition-all text-sm md:text-base ${mode === 'random' ? 'bg-white text-green-600 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
                  >
                    <RefreshCw className="w-3 h-3 md:w-4 md:h-4 mr-1 md:mr-2" /> Random
                  </button>
                  <button
                    onClick={() => { setMode('passphrase'); setPreviousPassword(null); }}
                    className={`flex-1 flex items-center justify-center py-2 md:py-3 px-2 md:px-4 rounded-xl font-bold transition-all text-sm md:text-base ${mode === 'passphrase' ? 'bg-white text-green-600 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
                  >
                ...
                <div>
                  <div className="flex justify-between items-center mb-4">
                    ...
                    <span className="text-2xl font-black text-green-600 bg-green-50 px-4 py-1 rounded-2xl border-2 border-green-100">{length}</span>
                  </div>
                  <input
                    type="range"
                    ...
                    className="w-full h-3 bg-gray-100 rounded-xl appearance-none cursor-pointer accent-green-600 border border-gray-200"
                  />
                </div>
                  ...
                  <div className="space-y-6">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 md:gap-4">
                      {[
                        ...
                        <label key={opt.id} className={`flex items-center p-3 md:p-4 rounded-2xl border-2 transition-all cursor-pointer group ${opt.state ? 'border-green-500 bg-green-50' : 'border-gray-100 hover:border-gray-200'} ${opt.full ? 'sm:col-span-2' : ''}`}>
                          <div className={`w-5 h-5 md:w-6 md:h-6 rounded-lg border-2 flex items-center justify-center mr-3 transition-all ${opt.state ? 'bg-green-500 border-green-500' : 'border-gray-300 group-hover:border-gray-400'}`}>
                          ...
                          <input
                            ...
                            className="hidden"
                          />
                          <span className={`font-bold text-sm md:text-base ${opt.state ? 'text-green-700' : 'text-gray-500'}`}>{opt.label}</span>
                        </label>
                      ))}
                    </div>
                    ...
                    <div className="space-y-4 pt-4 border-t border-gray-100">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 md:gap-4">
                        {[
                          ...
                          <label key={opt.id} className={`flex items-center p-3 md:p-4 rounded-2xl border-2 transition-all cursor-pointer group ${opt.state ? 'border-green-500 bg-green-50' : 'border-gray-100 hover:border-gray-200'}`}>
                            <div className={`w-5 h-5 md:w-6 md:h-6 rounded-lg border-2 flex items-center justify-center mr-3 transition-all ${opt.state ? 'bg-green-500 border-green-500' : 'border-gray-300 group-hover:border-gray-400'}`}>
                              {opt.state && <Check className="w-3 h-3 md:w-4 md:h-4 text-white" />}
                            </div>
                ...
                {mode === 'passphrase' && (
                  <div className="space-y-4 pt-4 border-t border-gray-100">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 md:gap-4">
                      <div className="flex items-center p-3 md:p-4 rounded-2xl border-2 border-gray-100 bg-white">
                        <span className="font-bold text-sm md:text-base text-gray-500 mr-auto">{t('pass.separator')}</span>
                        <select
                          value={passphraseSeparator}
                          onChange={(e) => setPassphraseSeparator(e.target.value)}
                          className="bg-gray-50 border border-gray-200 rounded-lg px-3 py-1 font-bold text-gray-700 focus:outline-none focus:border-green-500"
                        >
                ...
                <div className="pt-4 border-t border-gray-100">
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <select 
                      ...
                      className="bg-gray-50 border-2 border-gray-100 rounded-xl px-4 py-2 font-bold text-gray-700 focus:outline-none focus:border-green-500"
                    >
                ...
                <div className="flex flex-col sm:flex-row gap-4">
                  ...
                  {previousPassword && (
                    <button
                      ...
                      className="flex-1 bg-gray-100 hover:bg-gray-200 text-gray-600 font-bold py-5 px-6 rounded-2xl transition-all flex items-center justify-center text-base border-2 border-gray-200 active:scale-[0.98]"
                      title={t('pass.undo')}
                    >
          ...
          <div className="space-y-8">
            <div className="bg-white border border-gray-200 rounded-3xl shadow-lg p-6">
              <h3 className="font-black text-gray-900 flex items-center uppercase tracking-widest text-sm mb-6">
              ...
              <div className="space-y-4">
                {[t('pass.tip1'), t('pass.tip2'), t('pass.tip3'), t('pass.tip4')].map((tip, i) => (
                  <div key={i} className="flex items-start p-3 bg-gray-50 rounded-xl border border-gray-100">
                    ...
                    <p className="text-sm text-gray-600 leading-tight font-medium">{tip}</p>
                  </div>
                ))}
              </div>
            ...
            <div className="bg-green-600 rounded-3xl p-8 text-white shadow-xl shadow-green-100 relative overflow-hidden group">
              ...
              <ul className="space-y-4 text-green-50 text-sm leading-relaxed">
                <li className="flex items-start">
                  <span className="bg-white/20 rounded-lg p-1 mr-3 mt-0.5 font-bold text-xs">01</span>
                  {t('pass.help.1')}
                </li>
                <li className="flex items-start">
                  <span className="bg-white/20 rounded-lg p-1 mr-3 mt-0.5 font-bold text-xs">02</span>
                  {t('pass.help.3')}
                </li>
              </ul>
            </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-300">
          <div className="bg-white rounded-3xl p-8 max-w-sm w-full shadow-2xl animate-in zoom-in-95 duration-300">
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-xl font-black text-gray-900">{t('pass.qrCode')}</h3>
              <button onClick={() => setShowQr(null)} className="p-2 hover:bg-gray-100 rounded-xl transition-colors">
                <X className="w-6 h-6 text-gray-400" />
              </button>
            </div>
            <div className="bg-white p-6 border-4 border-gray-50 rounded-2xl flex justify-center mb-6">
              <QRCodeSVG value={showQr} size={200} level="H" includeMargin={true} />
            </div>
            <div className="bg-gray-50 p-4 rounded-xl border border-gray-100 mb-6">
              <code className="text-sm font-mono text-gray-600 break-all block text-center">{showQr}</code>
            </div>
            <button
              onClick={() => {
