          ...
          <div className="space-y-4">
            ...
            <div className="space-y-4 pb-4">
              {examples.map((example, i) => (
                <div key={i} className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-800 rounded-xl p-4 shadow-sm relative group">
                  <div className="absolute top-3 right-3 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button 
                      onClick={() => removeExample(i)}
                      disabled={examples.length <= 1}
                      className="p-1.5 text-slate-500 dark:text-slate-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-md disabled:opacity-30 disabled:cursor-not-allowed"
                    >
                      <Trash2 className="w-4 h-4" />
                    </button>
                  </div>
                  ...
                  <div className="space-y-3">
                    <div>
                      ...
                      <textarea
                        ...
                        className="w-full dark:bg-black/20 border border-slate-200 dark:border-white/[0.06] rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-orange-500/20 focus:border-orange-500 dark:text-white resize-y bg-slate-50 dark:bg-slate-900"
                        rows={2}
              ...
              <div className="bg-slate-800/80 px-4 py-3 border-b border-slate-700 flex justify-between items-center flex-wrap gap-3">
                <div className="flex bg-slate-900 rounded-lg p-1 border border-slate-700">
                  <button
                    ...
                    className={cn("px-3 py-1.5 rounded-md text-xs font-medium transition-colors", format === 'markdown' ? "bg-slate-700 text-white" : "text-slate-500 dark:text-slate-400 hover:text-slate-200")}
                  >
                    Markdown
                  </button>
                  <button
                    onClick={() => setFormat('xml')}
                    className={cn("px-3 py-1.5 rounded-md text-xs font-medium transition-colors", format === 'xml' ? "bg-slate-700 text-white" : "text-slate-500 dark:text-slate-400 hover:text-slate-200")}
                  >
                    XML
                  </button>
                  <button
                    onClick={() => setFormat('json')}
                    className={cn("px-3 py-1.5 rounded-md text-xs font-medium transition-colors", format === 'json' ? "bg-slate-700 text-white" : "text-slate-500 dark:text-slate-400 hover:text-slate-200")}
                  >
                    JSON
                  </button>
                </div>
